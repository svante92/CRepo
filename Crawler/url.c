...
char * normalizeURL(const char *base, const char *url) {
	...
	if (first_slash == NULL) {
		...
		if(url[0] == '/' && url[1] == '/')
		{
			out = calloc(schema_length + strlen(url) + 1, sizeof(char));
			if (out == NULL)
			{
			...
			if(base_is_http)
			{
				sprintf(out, "http:%s", url);
			}
			else if(base_is_https)
			{
		...
		else
		{
			...
			out = calloc(strlen(base) + (domain_relative? 0 : 1) + strlen(url) + 1, sizeof(char));
		...
		/* Check for fragment identifiers '#' and skip those URLs */
		if(strstr(out, "/#") != NULL)
		{
			free(out);
			out = NULL;
